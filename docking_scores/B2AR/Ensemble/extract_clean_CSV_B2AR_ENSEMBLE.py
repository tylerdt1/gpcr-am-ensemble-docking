...
# ── File paths ──────────────────────────────────────────────────────────────
SMILES_FILE = '/work/users/t/y/tylerdt/Schrodinger/LigPrep/b2AR-no_dup-ligprep/b2AR-no_dup-combined.smi'
OUT_DIR     = '/work/users/t/y/tylerdt/clean_ensemble/B2AR/'

GLIDE_DIR   = '/work/users/t/y/tylerdt/Schrodinger/Glide-HTVS-clusters/Enrichment-HTVS-clusters/b2AR-clusters-HTVS-lig_25-res-enrichment/Nclusters_10/'
VINA_DIR    = '/users/t/y/tylerdt/GPCR_Retrospective_Docking/AutoDockVina/B2AR/ensemble_predictions/enrichment/Nclusters_10/'
...
df[cols].to_csv(f'{OUT_DIR}B2AR_Ensemble_Glide_framewise_scores.csv', index=False)
...
df[['ligand_id', 'SMILES', 'BE_min', 'is_active']].to_csv(
    f'{OUT_DIR}B2AR_Ensemble_Glide_BEmin_ranked.csv', index=False)
...
df[['ligand_id', 'SMILES', 'BE_avg', 'is_active']].to_csv(
    f'{OUT_DIR}B2AR_Ensemble_Glide_BEavg_ranked.csv', index=False)
...
cols = ['ligand_id', 'SMILES'] + [f'cluster{i:02d}_BE' for i in range(10)] + ['is_active']
df[cols].to_csv(f'{OUT_DIR}B2AR_Ensemble_Vina_framewise_scores.csv', index=False)
print(f"  Framewise: {len(df)} compounds ({df['is_active'].sum()} actives)")

df = pd.read_csv(f'{VINA_DIR}BE_min_ranked.csv')
df = add_meta(df, 'BaseName')
df[['ligand_id', 'SMILES', 'BE_min', 'is_active']].to_csv(
    f'{OUT_DIR}B2AR_Ensemble_Vina_BEmin_ranked.csv', index=False)
print(f"  BEmin: {len(df)} compounds ({df['is_active'].sum()} actives)")

df = pd.read_csv(f'{VINA_DIR}BE_avg_ranked.csv')
df = add_meta(df, 'BaseName')
df[['ligand_id', 'SMILES', 'BE_avg', 'is_active']].to_csv(
    f'{OUT_DIR}B2AR_Ensemble_Vina_BEavg_ranked.csv', index=False)
print(f"  BEavg: {len(df)} compounds ({df['is_active'].sum()} actives)")
